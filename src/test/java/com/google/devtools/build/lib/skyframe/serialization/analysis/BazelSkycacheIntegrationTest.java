...
public final class BazelSkycacheIntegrationTest extends SkycacheIntegrationTestBase {
      ...
      when(client.lookup(any()))
          .thenAnswer(
              invocation -> {
                      ...
                      var deserializer =
                          new FileDependencyDeserializer(
                              new FingerprintValueService(
                                  ...
                                  null));
                      var dependencies =
                          switch (deserializer.getNestedDependencies(key)) {
                ...
                return immediateFuture(ByteString.copyFrom(entry, offset, entry.length - offset));
              });
    }
  ...
  public void incompleteAnalysisEntryFallsBackOnce(@TestParameter InvalidAnalysisEntry damage)
    ...
    var deps = getSkyframeExecutor().getRemoteAnalysisCachingDependenciesProvider();
    ...
    var serialized =
        codecs.serializeMemoizedAndBlocking(service, malformed, /* profileCollector= */ null);
    ...
    var fingerprint =
        SkyKeySerializationHelper.computeFingerprint(
            codecs, service, key, deps.getSkyValueVersion());
