...
multiple configurations produces multiple results, even if the formatted text is
identical. Expressions such as `some` and `somepath` retain cquery's freedom to
choose any matching target or path. Targets analyzed by earlier builds cannot
influence the query's scope.
